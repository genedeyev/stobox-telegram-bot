...
> **P3 (partial):** Postgres advisory leader lock (a second replica stands down instead of causing getUpdates 409 storms and doubled broadcasts); token-gated Prometheus `/metrics` on the web service; test-state isolation (test runs no longer mutate the repo's live `data/`); Telegram-surface test suite with PTB fakes (adapter 9%→36%, commands 12%→20%, total 61%→64%; 254 tests). Remaining open: engine decomposition, JSON books→Postgres, shared send/HTTP helper consolidation, further coverage push — sizable refactors best done as reviewed, standalone efforts.
