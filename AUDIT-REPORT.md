...
> **P3 (partial):** Postgres advisory leader lock (a second replica stands down instead of causing getUpdates 409 storms and doubled broadcasts); token-gated Prometheus `/metrics` on the web service; test-state isolation (test runs no longer mutate the repo's live `data/`); Telegram-surface test suite with PTB fakes (adapter 9%→36%, commands 12%→20%, total 61%→64%; 254 tests). **Engine decomposition done** (pure mechanical refactor, zero behavior change): `__init__` 128→42 lines via six named wiring methods; `handle` 149→57 via staged gates (`_moderation_gate`, `_remember_and_route`, `_record_fud`, `_declined`, `_pre_llm_gates`, `_reward_engagement`); `_answer` 186→39 via `_build_reply_prompt` / `_score_reply` / `_gate_low_confidence` / `_apply_output_rails`. **Books→Postgres solved via a state mirror**: files stay the sync working store; every atomic save mirrors to a `bot_state` table and boot restores missing ledgers from it — operational state now survives redeploys even with no volume (Railway). **Admin fan-out consolidated** into one `dm_admins` helper (11 call sites). Remaining open: coverage push toward 95% (currently 64%; adapter 36%, commands 20%) and the four hand-rolled HTTP client wrappers (cosmetic).
